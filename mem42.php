  ...
  // $out='列印客戶資料';
	...
	switch($out)
    {
		...
		case '修改客戶資料' :
			...
			if ($len>0)
			{
				$sql="select * from custom where  f_celephone= '$f_celephone' and f_passwd = '$f_passwd2' and sno !=" .$sno;  //判斷是否重複     
				$result1 =$mysqli->query($sql) or die($mysqli->connect_error);
				$sn_index=mysqli_num_rows($result);
				if ($sn_index >0) 
				{
			...
			$result =$mysqli->query($sql) or die($mysqli->connect_error); 
			$sn_index =mysqli_num_rows($result);   //查詢結果的記錄筆數	 $result->num_rows()		
			$row=$result->fetch_assoc();
        ...
        case '新增客戶資料' :
			...
			$result =$mysqli->query($sql) or die($mysqli->connect_error); 
			$sn_index = mysqli_num_rows($result);  //查詢結果的記錄筆數			
			if ($sn_index>0) 
			{
         ...
         case "chk" :	
			...
			case "編號查詢": 
				...
				break;
			case "手機查詢": 
				$sql_2 =" where f_celephone like '%$f_celephone%'";		
