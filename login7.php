 ...
 //  $out='登入';
	...
	switch($out)
    {
		...
		//	{
				...
				{
					$ins_flg="f1";
					$pri_echo = "此會員(EMAIL+密碼)已存在 或 Email 未填";
				}
		//	}
			$len=strlen($f_celephone2);
			if ($len>0)
        ...
        case '送出' :
			...
			$sql="insert into custom  (f_cust_mail,f_cust_id,f_passwd,f_cust_name,f_id,f_birthday,f_celephone,f_cust_tel,f_cust_address,f_line_id,f_introduction,f_c_date) values(";
			$sql.="'$f_email2',";
			$sql.="'" . $f_cust_id1 . "',";
			$sql.="'$f_passwd2',";       
			$sql.="'$f_user_name2',";    
...
//找第一層下線
		...
		case 'mail_check':			
			$display="n";			
			$sql ="select * from  custom where (f_cust_mail ='$f_email2') "; 
			$result =mysql_query($sql); 
			$sn_index = mysql_num_rows($result);   //查詢結果的記錄筆數 
			echo '<?xml version="1.0" encoding="utf8"?> ';
			if ($sn_index >0) 	 
