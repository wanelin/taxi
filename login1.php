 ...
 //  $out='登入';
    ...
    {		
		...
		case '完成修改司機資料' :
			...
			if ($len>0)
			{
				$sql="select * from driver where  f_driver_email = '$f_driver_email2' and f_passwd = '$f_passwd2' and sno != " . $sno;   //判斷是否重複     
				$result = $db->query($sql);		
			...
			if ($len>0)
			{
				$sql="select * from driver where  f_line_id= '$f_line_id2' and f_passwd = '$f_passwd2'and sno !=" .$sno;   //判斷是否重複     
				$result = $db->query($sql);		
				$sn_index= $result->rowCount(); //查詢結果的記錄筆數 		
			...
			if ($len>0)
			{
				$sql="select * from driver where  f_celephone= '$f_celephone2' and f_passwd = '$f_passwd2' and sno !=" .$sno;  //判斷是否重複     
				$result = $db->query($sql);		
				$sn_index= $result->rowCount(); //查詢結果的記錄筆數		
			...
			$sql="select * from driver where sno=$sno";  //判斷是否重複
		...
		case '帳號確認' :
			...
			$len=strlen($f_driver_email2);
			$pri_echo="";
				$sql="select * from driver where  f_driver_email = '$f_driver_email2'";   //判斷是否重複     
				...
				{
					$ins_flg="f1";
					$pri_echo = "此司機(EMAIL 已存在 或 Email 未填)";
				}
		    */
			$len=strlen($f_celephone2);
			if ($len>0)
			{
				//$sql="select * from driver where  f_celephone= '$f_celephone2' and f_passwd = '$f_passwd2'";   //判斷是否重複     
				$sql="select * from driver where  f_celephone= '$f_celephone2' ";   //判斷是否重複     
			...
			if ($len>0)
			{
				$sql="select * from driver where  f_line_id= '$f_line_id2' and f_passwd = '$f_passwd2'";   //判斷是否重複     
				$result = $db->query($sql);		
				$sn_index= $result->rowCount(); //查詢結果的記錄筆數
				if ($sn_index >0) 
				{
					$ins_flg="f3";
					$pri_echo .= "此司機(line+密碼)已存在";
				}
			}
            */			
			$very_code="";
...
//	$SendGet->sendRequest(); //發送
	//$a=$SendGet->getResponseBody(); //取回傳值
		...
		case '新增司機送出' :
			...
			$month=date("m");
			$defaut_date=$year.$month;					
			$sql="select * from driver where f_driver_id like '$defaut_date%' order by f_driver_id DESC";  //判斷是否有使用者帳號    
			...
			$sql = "INSERT INTO `driver` (f_driver_email,f_driver_id,f_passwd,f_driver_name,f_birthday,f_celephone,f_driver_tel,f_driver_address,f_c_date 
...
//處理相關設備
        ...
        case '登入':						
			...
			{
			case "0" :
				$sql="select * from driver where  (f_celephone = '$f_celephone1') ";  //判斷是否重複
				break;
			case "1" :
				$sql ="select * from  driver where (f_driver_email ='$f_celephone1') "; 				     
				break;
			case "2" :
				$sql="select * from driver where  (f_line_id = '$f_celephone1') ";  //判斷是否重複
				break;
			}
			...
			else
			{
				$display="y";
				$_SESSION["passok"]="n";
				$sql ="select * from  driver where f_celephone = '$f_celephone1'";
				$result = $db->query($sql);		
				$sn_index= $result->rowCount(); //查詢結果的記錄筆數 
				if ($sn_index >0) 	 
				{ //判斷是否重複 
		...
		case '修改司機登入':			
			...
			{
			case "0" :
				$sql="select * from driver where  (f_celephone = '$f_driver_id1') ";  //判斷是否重複
				break;
			case "1" :
				$sql ="select * from  driver where (f_driver_email ='$f_driver_id1') "; 				     
				break;
			case "2" :
				$sql="select * from driver where  (f_line_id = '$f_driver_id1') ";  //判斷是否重複
				break;				
			}
			$sql.=" and (f_passwd  = '$f_passwd1')";
			$result = $db->query($sql);		
			...
			{ //判斷是否重複
				...
				$f_driver_id=$row["f_driver_id"];   //司機編號
				$f_driver_name=$row["f_driver_name"];					 
