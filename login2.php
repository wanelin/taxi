...
// XML文件
  ...
  //接收前台傳來的資料	
	$f_custom_id      =trim($_POST["f_custom_id"]);                  
	$f_custom_name    =trim($_POST["f_custom_name"]);  	
	$f_custom_tel     =trim($_POST["f_custom_tel"]);
	$f_custom_address =trim($_POST["f_custom_address"]);  
	$f_celephone1      =trim($_POST["f_celephone1"]);  
	$f_celephone2      =trim($_POST["f_celephone2"]);  
	$f_custom_email1   =trim($_POST["f_custom_email1"]);
	$f_custom_email2   =trim($_POST["f_custom_email2"]);
	$f_custom_area    =$_POST["f_custom_area"];
	...
	$f_custom_id_top      	=$_POST["f_custom_id_top"];
 ...
 //  $out='登入';
	...
	$up_line="";
	$down_line="";
	function pic_upload($f_custom_id_buf)  //上傳圖片
	{
	...
	foreach($_FILES as $file) //將每個圖檔上傳
	{
		...
		$file_name=$f_custom_id_buf;
		if($pic_type[$j]<3)
		{
			$file_name=$f_custom_id_buf . "_" . $pic_type[$j];
		}				
    ...
    {		
		case '修改客戶資料' :		
			...
			if ($update_flg =="t") 
			{				
				$input = array(':f_custom_email'  		=> $f_custom_email2, 
							   ':f_passwd'      		=> $f_passwd2,
							   ':f_celephone'    		=> $f_celephone2,  
							   ':f_custom_tel'     		=> $f_custom_tel,   	
							   ':f_custom_address'    	=> $f_custom_address, 
							   ...
							   ':sno'                   => $sno);				
				$sql = "UPDATE custom 
						SET f_custom_email  	 	=:f_custom_email,  	    	
						    f_passwd         		=:f_passwd,         		
		...
		case '完成修改客戶資料' :
			...
			$update_flg="f";			
			$len=strlen($f_custom_email2);
			if ($len>0)
			{
				$sql="select * from custom where  f_custom_email = '$f_custom_email2' and f_passwd = '$f_passwd2' and sno != " . $sno;   //判斷是否重複     
				$result = $db->query($sql);		
				...
				{
					$update_flg="f1";
					$pri_echo = "此客戶(EMAIL+密碼)別的客戶已使用" .$sql;
				} 
				else
				{
					$update_flg="t";
			...
			if ($len>0)
			{
				...
				{
					$update_flg="f2";
					$pri_echo .= "此客戶(line+密碼)別的客戶已使用" .$sql;
				} 
				else
				{
					$update_flg="t";					
			...
			{				
				...
				$sql.="f_custom_email='$f_custom_email2',";					
				$sql.="f_passwd='$f_passwd2',";       
				$sql.="f_custom_name='$f_custom_name2',";    
				$sql.="f_id='$f_id2',";           
				$sql.="f_birthday='$f_birthday2',";     
				$sql.="f_celephone='$f_celephone2',";
				$sql.="f_custom_tel='$f_custom_tel2',";					
				$sql.="f_custom_address='$f_address2',";      
				$sql.="f_line_id='$f_line_id2',";      
				$sql.="f_introduction='$f_sug_custom_id2',";						
			...
			$f_custom_id1=$row["f_custom_id"]; 
			$display="n";	
			echo '<?xml version="1.0" encoding="utf8"?> ';
			echo "<factory>";
			echo "<pri_echo>$pri_echo</pri_echo>";
			echo "<f_custom_id>$f_custom_id1</f_custom_id>";
			echo "</factory>";
			break;
		case '帳號確認' :
			$ins_flg="f";
			...
			$len=strlen($f_custom_email2);
			$pri_echo="";
				$sql="select * from custom where  f_custom_email = '$f_custom_email2'";   //判斷是否重複     
				...
				{
					$ins_flg="f1";
					$pri_echo = "此客戶(EMAIL 已存在 或 Email 未填)";
				}
		    */
			$len=strlen($f_celephone2);
			if ($len>0)
...
//	$SendGet->sendRequest(); //發送
	//$a=$SendGet->getResponseBody(); //取回傳值
		...
		case '新增客戶送出' :
			...
			$month=date("m");
			$defaut_date=$year.$month;					
			$sql="select * from custom where f_custom_id like '$defaut_date%' order by f_custom_id DESC";  //判斷是否有使用者帳號    
			...
			if ($sn_index>0) 
			{
				...
				$f_custom_id=$f_custom_id_int+1;
			}
			else
			{					
				$f_custom_id=$year . $month . "0001";					
			}						
			$f_custom_id=strtolower($f_custom_id);			
			
			$input = array(':f_custom_email'=>$f_custom_email2,':f_custom_id' =>$f_custom_id,':f_passwd' =>$f_passwd2, 
			               ':f_custom_name'=>$f_custom_name,':f_birthday'=>$f_birthday,':f_celephone'=>$f_celephone2,
						   ':f_custom_tel'=>$f_custom_tel,':f_custom_address'=>$f_custom_address,':f_c_date'=>$f_c_date,
			...
			$sql = "INSERT INTO `custom` (f_custom_email,f_custom_id,f_passwd,f_custom_name,f_birthday,f_celephone,f_custom_tel,f_custom_address,f_c_date 
										,f_contact_number,f_registration_number,f_weight,f_length,f_width,f_height,f_property)
								  VALUES(:f_custom_email,:f_custom_id,:f_passwd,:f_custom_name,:f_birthday,:f_celephone,:f_custom_tel,:f_custom_address, 
...
//處理相關設備
			/*
			$sql = "DELETE FROM `equitment` WHERE `f_custom_id` = '$f_custom_id'";			
			$sth = $db->prepare($sql);			
			$sth->execute(array(':f_custom_id'=>$f_custom_id));
			
			for ($i=0;$i<count($f_equitment);$i++)
			{				
				$input = array(':f_custom_id' =>$f_custom_id,':f_equitment' =>$f_equitment[$i]); 
				$sql = "INSERT INTO `equitment` (f_custom_id,f_equitment)
								  VALUES(:f_custom_id,:f_equitment)";
				$sth = $db->prepare($sql);			
				
				$sth->execute($input);
			} 
		    pic_upload($f_custom_id);
			*/
			$pri_echo =$f_custom_id."客戶新增完成"   ;
			echo '<?xml version="1.0" encoding="utf8"?> ';
			echo "<factory>";
			echo "<pri_echo>$pri_echo</pri_echo>";	
        ...
        case '登入':						
			...
			case "0" :
				...
				break;
			case "1" :
				$sql ="select * from  custom where (f_custom_email ='$f_celephone1') "; 				     
				break;
			case "2" :
			...
			{ 
				...
				$f_custom_email1= $row["f_custom_email"];
				$f_custom_id= $row["f_custom_id"];
				$f_custom_name= $row["f_custom_name"];
				$f_area= $row["f_custom_area_code"];
				$f_custom_address= $row["f_custom_address"];
				...
				echo "<f_custom_email>$f_custom_email1</f_custom_email>";
				echo "<f_custom_id>$f_custom_id</f_custom_id>";
				echo "<f_custom_name>$f_custom_name</f_custom_name>";
				echo "<f_custom_address>$f_custom_address</f_custom_address>";
				echo "<f_custom_area>$f_area</f_custom_area>";
				...
				echo "<action>y</action>";	
				/*
				$input = array(':f_custom_id' =>$f_custom_id);
				$sql_1  = "select * from equitment  where f_custom_id = '$f_custom_id'";
				...
				*/
				...
				echo "</sales>";
				$_SESSION["passok"]="y";
				$_SESSION["f_custom_id"]=$f_custom_id1;
				$_SESSION["f_custom_name"]=$f_custom_name1;
		...
		case '修改客戶登入':			
			...
			{
			case "0" :
				$sql="select * from custom where  (f_celephone = '$f_custom_id1') ";  //判斷是否重複
				break;
			case "1" :
				$sql ="select * from  custom where (f_custom_email ='$f_custom_id1') "; 				     
				break;
			case "2" :
				$sql="select * from custom where  (f_line_id = '$f_custom_id1') ";  //判斷是否重複
				break;				
			}
			$sql.=" and (f_passwd  = '$f_passwd1')";
			$result = $db->query($sql);		
			...
			{ //判斷是否重複
				...
				$f_custom_id=$row["f_custom_id"];   //客戶編號
				$f_custom_name=$row["f_custom_name"];					 
				...
				$sno=$row["sno"];   //
				$_SESSION["passok"]="y";
				$_SESSION["f_custom_id"]=$f_custom_id;
				$_SESSION["f_custom_name"]=$f_custom_name;
				$pri_echo ="y";
				echo "<factory>";
	...
	if ($display=="y")	
	{
		...
		echo "<f_custom_id>$f_custom_id1</f_custom_id>";
		echo "<f_custom_name>$f_custom_name1</f_custom_name>";	
		echo "</factory>";
	}
?>